...
static int gf_request_resources(struct gf_dev *gf_dev)
{
	int rc;
...
static void gf_release_resources(struct gf_dev *gf_dev)
{
	/* Just revert wake-up ability of the IRQ to default */
...
int gf_probe_common(struct device *dev)
{
	...
	rc = gf_request_resources(gf_dev);
	if (rc < 0)
		return rc;
